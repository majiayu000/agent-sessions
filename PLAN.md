...
| T5 | remem | 保留 raw 身份/完整性/过滤边界；spec 在先；focused/full tests、隔离导入 smoke、版本同步/preflight | 完成 eb5fd714；4078生产测试与最终eval/native证据通过；Draft PR1091待CI/合并 |
...
| T9 | keepline / TS 文档 | Rust history 读取迁移，保留历史条目计数与错误显示；Rust/相关 TS checks；共享格式/fixture 文档 | 完成157c309，Draft PR116；待ccstats registry发布 |
| T10 | life-looper | ccstats CLI 版本/JSON/时区/范围/错误/超时/配置定价契约成立；Go tests；不再自行解析或硬编码降级价格 | 完成f832912，PR1；release CLI联测通过，待合并/依赖发布 |
